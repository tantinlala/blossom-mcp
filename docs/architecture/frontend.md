...
- **TaskDetailsForm**: The task's name, description and completion state, with an Update button that applies the edit to the task. The description reads as formatted text carrying clickable links, and clicking it (or pressing Enter on it) swaps in a textarea; blur or Escape returns to the reading view. `TaskDetailsDrawer` keys the form on the task id, so switching tasks lands on the reading view.
